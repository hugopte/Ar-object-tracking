...
def main():
    ...
    dir_name = os.getcwd()
    model = cv2.imread(os.path.join(dir_name, 'reference\\picture.jpg'), 0)
...
def main2():
    ...
    homography = None 
    # matrix of camera parameters (made up but works quite well for me) 
    camera_parameters = np.array([[1600, 0, 320], [0, 800, 240], [0, 0, 1]])
    # create ORB keypoint detector
    dir_name = os.getcwd()
    orb = cv2.ORB_create()
    # create BFMatcher object based on hamming distance  
    bf = cv2.BFMatcher(cv2.NORM_HAMMING, crossCheck=True)
    # load the reference surface that will be searched in the video stream
    model = cv2.imread(os.path.join(dir_name, 'reference\\photo.png'), 0)
    print(os.path.join( 'reference/photo.png'))    # Compute model keypoints and its descriptors
    kp_model, des_model = orb.detectAndCompute(model, None)
    # Load 3D model from OBJ file
    ...
    while True:
        ...
        if len(matches) > MIN_MATCHES and len(matches) < 80:
            print("matches found - %d/%d" % (len(matches), MIN_MATCHES))

            orb2 = cv2.ORB_create()

            # find the keypoints with ORB
            kp = orb2.detect(frame, None)

            # compute the descriptors with ORB
            kp, des = orb2.compute(frame, kp)

            # draw only keypoints location,not size and orientation
            frame = cv2.drawKeypoints(frame, kp, frame, color=(0, 255, 0), flags=0)
            model = cv2.drawKeypoints(model, kp_model,model, color=(0, 255, 0), flags=0)
            cv2.imshow('frame', frame)
            cv2.imshow('test', model)
